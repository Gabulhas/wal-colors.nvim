...
function Color.black() return Color:new(0, 0, 0) end

function Color:clone() return Color:new(self.hue, self.saturation, self.value) end

function Color:darkened(ratio)
    if ratio == nil then ratio = 0.1 end
    ratio = 1 - ratio
...
function Color:mixed_hsv(other, ratio)
    if ratio == nil then ratio = 0.5 end
    ...
    local new_color = Color:new(new_hue, self.saturation * (1 - ratio) +
                                    other.saturation * ratio,
