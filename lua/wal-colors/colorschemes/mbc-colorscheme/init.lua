...
M.highlights_base = function(colors)
    ...
    local old_background = colors.background

    local highlights = {}

    local function set_hl(name, hl) highlights[name] = hl end

    local function get_hl(name) return highlights[name] end

    for _, config in ipairs(configs) do config.setup(set_hl, get_hl, colors) end

    colors.background = old_background
    ...
    set_hl("SignColumn",
           {bg = colors.background:darkened(0.1), fg = colors.background})
    ...
    set_hl("LspReferenceText", {bg = colors.background:highlighted(0.2)})
    set_hl("LspReferenceRead",
           {bg = colors.background:highlighted(0.2), italic = true})
    set_hl("LspReferenceWrite",
           {bg = colors.background:highlighted(0.2), bold = true})
    ...
    set_hl("NeogitHunkHeaderHighlight",
           {bg = colors.foreground, fg = colors.color7})
    set_hl("NeogitDiffContextHighlight",
           {bg = colors.background, fg = colors.foreground})
    set_hl("NeogitDiffDeleteHighlight",
           {fg = colors.color11, bg = colors.color11})
    ...
    -- GitSigns
    set_hl("GitSignsAdd",
           {fg = colors.color4, bg = colors.background:darkened(0.1)}) -- diff mode: Added line |diff.txt|
    set_hl("GitSignsChange",
           {fg = colors.color5, bg = colors.background:darkened(0.1)}) -- diff mode: Changed line |diff.txt|
    set_hl("GitSignsDelete",
           {fg = colors.color11, bg = colors.background:darkened(0.1)}) -- diff mode: Deleted line |diff.txt|
    ...
    -- set_hl("IblIndent", { bg = colors.background })
    set_hl("IblScope", {fg = colors.color4:darkened(0.3)})
    ...
    set_hl("NvimTreeStatusLineNC",
           {bg = colors.background, fg = colors.background})

    -- Scrollbar
    set_hl("ScrollbarHandle", {
        fg = colors.background:lightened(0.3),
        bg = colors.background:lightened(0.3)
    })
