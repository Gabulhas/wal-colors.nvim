...
M.setup = function(set_hl, get_hl, colors)
    set_hl("PMenu", {link = "NormalFloat", bg = colors.bright_black})
    set_hl("CmpGhostText", {fg = colors.bright_black})

    set_hl("CmpItemAbbrDeprecated",
           {strikethrough = true, bg = colors.bright_black})
    ...
    set_hl("CmpItemKindCopilot", {
        bg = colors.red:mixed_hsv(colors.blue):desaturated(0.3):darkened(0.3)
    })
    set_hl("CmpItemKindCodeium", {
        bg = colors.red:mixed_hsv(colors.blue):desaturated(0.3):darkened(0.3)
    })
end
